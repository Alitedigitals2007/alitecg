...
function mapUser(row) {
  return { id: row.id, uniqueCode: row.unique_code, fullName: row.full_name, school: row.school, phoneNumber: row.phone_number, submissionType: row.submission_type, proofImageUrl: row.proof_image_url, status: row.status, rejectionReason: row.rejection_reason, vcfFormattedName: row.vcf_formatted_name, createdAt: row.created_at };
}
...
app.post('/submit', upload.single('proof'), wrap(async (req, res) => {
  ...
  const { rows } = await pool.query(
    'INSERT INTO submissions (unique_code, full_name, school, phone_number, submission_type, proof_image_url) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
    [uniqueCode, fullName.trim(), school, phoneNumber.trim(), submissionType, `/uploads/${req.file.filename}`]
  );
